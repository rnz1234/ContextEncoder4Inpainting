...
class ImagesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.masking_method == MaskingMethod.CentralRegion:
            masked_image = self._mask_central_region(image)
        elif self.masking_method == MaskingMethod.RandomBlock:
            masked_image = self._mask_random_block(image)
        elif self.masking_method == MaskingMethod.RandomRegion:
            masked_image = self._mask_random_region(image)
        else:
        ...
        import pdb
        pdb.set_trace()
        return {"orig_image" : image, "masked_image" : masked_image}
